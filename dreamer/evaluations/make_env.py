...
from racecar_gym.envs.gym_api import ChangingTrackMultiAgentRaceEnv, MultiAgentRaceEnv
from racecar_gym.envs.gym_api.multi_agent_race import MultiAgentScenario
from callbacks import save_eval_videos, save_episodes, save_trajectory, summarize_episode, summarize_eval_episode


def make_multi_track_env(tracks, action_repeat, render_mode=None, is_dreamer=True):
  ...
  env = ChangingTrackMultiAgentRaceEnv(scenarios=[f'scenarios/eval/{track}.yml' for track in tracks], order='manual', render_mode=render_mode)
...
def make_single_track_env(track, action_repeat, render_mode=None):
  env = MultiAgentRaceEnv(scenario=f'scenarios/eval/{track}.yml', render_mode=render_mode)
  env = wrappers.RaceCarWrapper(env)
  env = wrappers.FixedResetMode(env, mode='grid')
  env = wrappers.ActionRepeat(env, action_repeat)
  env = wrappers.ReduceActionSpace(env, low=[0.005, -1.0], high=[1.0, 1.0])
